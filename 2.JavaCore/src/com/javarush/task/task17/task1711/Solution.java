                ...
                synchronized (allPeople) {
                    for (int i = 1; i < args.length; i += 3) {
                        if (args[i + 1].equals("м"))
                            allPeople.add(Person.createMale(args[i], convertStringToDate(args[i + 2])));
                        else if (args[i + 1].equals("ж"))
                            allPeople.add(Person.createFemale(args[i], convertStringToDate(args[i + 2])));
            ...
            case "-u":
                synchronized (allPeople) {
                    for (int i = 1; i < args.length; i += 4) {
                        ...
                        person.setSex("м".equals(args[i+2]) ? Sex.MALE : Sex.FEMALE);
                        person.setBirthDay(convertStringToDate(args[i+3]));
                    }
                    break;
                }
            case "-d":
            ...
            case "-i":
                ...
                synchronized (allPeople){
                    for (int i = 1; i <= args.length-1; i++){
                        ...
                        System.out.println(person.getName() + " " + sex + " " + formDateOut.format(person.getBirthDay()));
                    }
                    break;
                }
        }
